...
public interface IPracticeApi
{
    ...
    Task<AppointmentProfileResponseList> DoctorAppointmentProfileList([Query] Guid practionerId,
        [Query] Guid organizationId, [Query] Guid practiceId, [Header("Authorization")] string token);
    ...
    Task<List<ProviderSearchResponse>> DoctorCalendarSlots([Body] ProviderSearchRequest request,
        [Header("Authorization")] string token);

    [Post("/api/Doctor/CalendarProviders")]
    Task<List<Guid>> DoctorCalendarProviders([Body] ProviderSearchRequest request,
        [Header("Authorization")] string token);

    [Get("/api/Doctor/DoctorSetupList")]
    Task<PagedResults<DoctorSetupDetails>> DoctorDoctorSetupList([Header("Authorization")] string token,
        Guid organizationId, string q = "", bool? showArchived = null, int pageNumber = 1, int pageSize = 10);

    [Get("/api/Staff/StaffList?OrganizationId={organizationId}&Archive={archive}")]
    Task<List<DoctorSetupDetails>> StaffStaffList(Guid organizationId, bool archive,
        [Header("Authorization")] string token);
    ...
    Task<PagedResults<DoctorSetupDetails>> PractitionerRolesList([Header("Authorization")] string token,
        Guid organizationId, string types = "", string q = "", bool? showArchived = null, int pageNumber = 1,
        int pageSize = 10);

    [Post("/api/Doctor/CalendarSlots2")]
    Task<List<ProviderSearchResponse>> DoctorCalendarSlots2([Body] ProviderSearchRequest request,
        [Header("Authorization")] string authorization);

    [Post("/api/Doctor/CalendarProviders2")]
    Task<List<Guid>> DoctorCalendarProviders2([Body] ProviderSearchRequest request,
        [Header("Authorization")] string authorization);
    ...
    Task<List<UpdateExamRoomApiRequest>> GetPracticeRooms(Guid practiceId,
        [Header("Authorization")] string authorization);
}